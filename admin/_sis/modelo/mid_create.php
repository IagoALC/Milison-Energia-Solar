...
if ($MidiaId) :
    ...
    if ($Read->getResult()) :
        ...
        $Read->ExeRead(DB_USERS, "WHERE user_id = :user", "user={$midia_user_id}");
        if ($Read->getResult()) :
            ...
            header('Location: dashboard.php?wc=modelo/mid_home');
            exit;
        endif;
    else :
        $_SESSION['trigger_controll'] = Erro("<b>OPPSS {$Admin['user_name']}</b>, você tentou editar uma mídia que não existe ou que foi removido recentemente!", E_USER_NOTICE);
    ...
    $NewMidia = ['midia_title' => '',
                 'midia_user_id' => 1];
    $Create->ExeCreate(DB_MIDIAS, $NewMidia);
    header('Location: dashboard.php?wc=modelo/mid_create&id=' . $Create->getResult());
    exit;
endif;
?>
    ...
    <div class="w-100 d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        <div class="d-flex align-items-center flex-wrap mr-1">
            <div class="d-flex align-items-baseline flex-wrap mr-5">
                <h5 class="text-dark font-weight-bold my-1 mr-5">Gestão de Mídias</h5>
                <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                    <li class="breadcrumb-item text-muted">
                        <a href="dashboard.php?wc=home" class="text-muted">Dashboard</a>
                    </li>
                    <li class="breadcrumb-item text-muted">
                        <a href="dashboard.php?wc=modelo/mid_home" class="text-muted">Mídias</a>
                    </li>
                    <li class="breadcrumb-item text-muted">
            ...
            <div class="card card-custom">
                <div class="card-header py-3">
                    ...
                    <div class="card-toolbar">
                        <span rel='dashboard_header_search' callback='Midias' callback_action='delete' class='btn btn-danger j_swal_action' id='<?= $MidiaId; ?>'>Deletar Registro!</span>
                    </div>
                </div>
                <form class="auto_save j_tab_home tab_create" name="user_manager" action="" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="midia_id" value="<?= $MidiaId; ?>" />
                    <div class="card-body">
                        <div class="form-group row">
                            <div class="col-lg-12">
                                <label>Título:</label>
                                <input class="form-control" value="<?= $midia_title; ?>" type="text" name="midia_title" placeholder="Título" />
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-lg-12">
                                <label>Descrição:</label>
                                <textarea class="form-control work_mce_basic" type="text" name="midia_desc" rows="10"><?= $midia_desc; ?></textarea>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-lg-6">
                                <label>Status:</label>
                                <select class="form-control" name="midia_status" id="">
                            ...
                            <div class="col-lg-6">
                                ...
                                <select name="midia_user_id" class="form-control" id="">
                                <?php
                        ...
                        <div class="form-group row">
                            <div class="col-lg-12">
                                <label>Link:</label>
                                <input class="form-control work_mce_basic" type="text" name="midia_link" rows="10"><?= $midia_link; ?></input>
                            </div>
                        </div>
                    </div>
