...
$NivelAcess = 3;//LEVEL_WC_CONFIG_CODES;
        ...
        case 'workcodes':
            if (empty($PostData['code_name']) || empty($PostData['code_script'])):
                ...
                if (empty($PostData['code_id'])):
                    ...
                    $jSON['trigger'] = AjaxErro("<b>CADASTRO COM SUCESSO:</b> O seu CODE foi cadastrado com sucesso e você já pode ver a alteração em seu site!");
                else:
                    $CodeId = $PostData['code_id'];
                    unset($PostData['code_id']);
                    $Update->ExeUpdate(DB_WC_CODE, $PostData, "WHERE code_id = :id", "id={$CodeId}");
                    $jSON['trigger'] = AjaxErro("<b>ATUALIZADO COM SUCESSO:</b> O seu CODE foi atualizado com sucesso e você já pode ver a alteração em seu site!", E_USER_NOTICE);
                endif;
            endif;
            break;
