...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
    ...
    switch ($Case):     
        ...
        case 'manager':
            ...
            if ($Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPSS:</b> Olá {$_SESSION['userLogin']['user_name']}. O e-mail <b>{$PostData['user_email']}</b> já está cadastrado na conta de outro Fornecedor!", E_USER_WARNING);
            else:
                ...
                $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>TUDO CERTO:</b> Olá {$_SESSION['userLogin']['user_name']}. O Fornecedor {$PostData['user_name']} {$PostData['user_lastname']} foi atualizado com sucesso!");
        ...
        case 'delete':
            ...
            if (!$Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>Fornecedor NÃO EXISTE:</b> Olá {$_SESSION['userLogin']['user_name']}, você tentou deletar um Fornecedor que não existe ou já foi removido!", E_USER_WARNING);
            else:
                ...
                elseif ($user_level > $_SESSION['userLogin']['user_level']):
                    ...
                    $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>Fornecedor REMOVIDO COM SUCESSO!</b>");
                    $jSON['redirect'] = "dashboard.php?wc=fornecedores/home";
                endif;
            endif;
            break;
