    ...
    switch ($Case):
        ...
        case 'admin_login':
            if (in_array('', $PostData)):
                ...
                elseif (strlen($PostData['user_password']) < 5):
                    ...
                    $Read = new Read;
                    $Read->FullRead("SELECT user_id FROM " . DB_USERS . " WHERE user_level >= :lv", "lv=3");
                    if (!$Read->getResult()):
                    ...
                    if (!$Read->getResult()):
                        ...
                        if (!$Read->getResult()):
                            ...
                            $Read->ExeRead(DB_USERS, "WHERE user_email = :email AND user_password = :pass AND user_level >= :level", "email={$PostData['user_email']}&pass={$PostData['user_password']}&level=3");
                            if (!$Read->getResult()):
        ...
        case 'admin_recover':
            if (isset($PostData['user_email']) && Check::Email($PostData['user_email']) && filter_var($PostData['user_email'], FILTER_VALIDATE_EMAIL)):
                $Read = new Read;
                $Read->FullRead("SELECT user_id, user_name, user_email, user_password FROM " . DB_USERS . " WHERE user_email = :email AND user_level >= :level", "email={$PostData['user_email']}&level=3");
                if (!$Read->getResult()):
