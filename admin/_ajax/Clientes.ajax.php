...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack) :
    ...
    switch ($Case):
        ...
        case 'manager':
            ...
            if (isset($PostData["especialidades"])) :
                $Delete->ExeDelete(DB_USER_ESPECIALIDADES, "WHERE user_id =:id", "id={$UserId}");
                foreach ($PostData["especialidades"] as $Reg) :
                    $Create->ExeCreate(DB_USER_ESPECIALIDADES, ["user_id" => $UserId, "especialidade_id" => $Reg]);
                endforeach;
            endif;
            unset($PostData['especialidades']);

            if (isset($PostData["idiomas"])) :
                $Delete->ExeDelete(DB_USER_IDIOMAS, "WHERE user_id =:id", "id={$UserId}");
                foreach ($PostData["idiomas"] as $Reg) :
                    $Create->ExeCreate(DB_USER_IDIOMAS, ["user_id" => $UserId, "idioma_id" => $Reg]);
                endforeach;
            endif;
            unset($PostData['idiomas']);
            ...
            $jSON['redirect'] = "dashboard.php?wc=modelo/home";
            break;
        ...
        case 'addr_delete':
            $Read->ExeRead(DB_USERS_ADDR, "WHERE addr_id = :addr", "addr={$PostData['id']}");
            if (!$Read->getResult()) :
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>ENDEREÇO NÃO EXISTE:</b> Olá {$_SESSION['userLogin']['user_name']}, você tentou deletar um Endereço que não existe ou já foi removido!", E_USER_WARNING);
            else :
                extract($Read->getResult()[0]);
                $Delete->ExeDelete(DB_USERS_ADDR, "WHERE addr_id = :addr", "addr={$PostData['id']}");
                $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>ENDEREÇO REMOVIDO COM SUCESSO!</b>");
                $jSON['redirect'] = "dashboard.php?wc=clientes/end_home&id={$user_id}";
            endif;
            break;
    endswitch;
